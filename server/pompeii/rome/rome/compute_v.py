...
def compute_v(
    ...
    logging.info(f"Rewrite layer is {layer}")
    logging.info(f"Tying optimization objective to {loss_layer}")
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.mlp_module_tmp.format(layer):
            ...
            if target_init is None:
                logging.info("Recording initial value of v*")
    ...
    logging.info(f"Delta norm: {(target - cur_output).norm().item()}")
    logging.info(
        f"Change in target norm: {target_init.norm().item()} to {target.norm().item()} => {(target.norm() - target_init.norm()).item()}"
    )
    logging.info(f"Division Factor: {torch.dot(cur_input, left_vector).item()}")
    logging.info(f"Right vector norm: {right_vector.norm()}")
